...
void UDPSocket::udpClientBind(UDP_TYPE udptype, const string& address = 0, unsigned short port = 8888)
{
    switch (udptype)
    {
    case UDP_TYPE::UDP_UNICAST:
        UDPSocket::unicastUDPClient(address, port);
        break;
    case UDP_TYPE::UDP_MULTICAST:
        UDPSocket::multicastUDPClient(address, port);
        break;
    case UDP_TYPE::UDP_BROADCAST:
...
int UDPSocket::Listen(char* message)
{
    int msglen = RecvFrom(message, sizeof(message));
